  ...
  return (
    <div className="animate-in fade-in duration-500 pb-10">
            ...
            <div key={item.id} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
              ...
              <div className="absolute bottom-6 left-4 right-4 md:left-12 z-20 font-nunito animate-in slide-in-from-bottom-10 duration-700">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => onMangaClick(item.id)} className="flex-1 max-w-[160px] bg-gradient-to-r from-[#CC0000] to-[#7A0000] text-white py-3 rounded-xl font-bold text-lg flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(204,0,0,0.5)] hover:scale-105 transition-all font-teko tracking-wider border border-[#FF3333]/30">
      ...
      <div className={carouselData.length === 0 ? "pt-24" : ""}>
        ...
        <Shelf title="Adicionados Recentemente" data={obrasRecentes} color="#FF3333" badge="Novo" onBookmark={(id) => setSaveModal({ isOpen: true, obraId: id })} onMangaClick={onMangaClick} />
        <Shelf title="Últimas Atualizações" data={obrasAtualizadas} color="#7A3CFF" isUpdate={true} onBookmark={(id) => setSaveModal({ isOpen: true, obraId: id })} onMangaClick={onMangaClick} />
      </div>
    </div>
  );
});

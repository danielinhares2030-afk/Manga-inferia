...
const UpdateCard = ({ obra, onMangaClick }) => {
  const [caps, setCaps] = useState([]); // Agora guarda uma lista de capítulos
  ...
  useEffect(() => {
    const fetchCaps = async () => {
      try {
        ...
        // Pega apenas os 2 últimos capítulos para exibir
        setCaps(lista.slice(0, 2)); 
      } catch (err) {
        console.error(err);
      }
    };
  ...
  return (
    <div onClick={() => onMangaClick(obra.id)} className="flex flex-col bg-[#0A0505] border border-[#2A0A0A] rounded-2xl hover:border-[#CC0000]/50 transition-colors cursor-pointer group shadow-lg overflow-hidden relative">
      <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-gradient-to-b from-[#CC0000] to-transparent"></div>
      
      {/* Topo do Card: Imagem e Título */}
      <div className="flex w-full p-3 pl-4 gap-3">
        <img src={obra.capaUrl} alt={obra.nome} className="w-20 h-[110px] object-cover shrink-0 rounded-lg border border-[#1A0505] shadow-md" />
        
        <div className="flex-1 flex flex-col min-w-0">
          <span className="text-[8px] text-[#CC0000] font-black uppercase tracking-widest bg-[#CC0000]/10 px-1.5 py-0.5 rounded border border-[#CC0000]/20 inline-block w-fit mb-1.5">
            {obra.tipo}
          </span>
          <h4 className="font-nunito text-sm font-bold text-[#F5F7FF] group-hover:text-white line-clamp-2 leading-tight break-words">
            {obra.nome}
          </h4>
        </div>
      </div>

      {/* Base do Card: Os 2 Capítulos */}
      <div className="px-3 pb-3 pl-4 flex flex-col gap-1.5 w-full mt-auto">
        {caps.length > 0 ? (
          caps.map((cap) => {
            const timeLabel = formatTimeAgo(cap.dataAdicionado || obra.updatedAt);
            return (
              <div key={cap.id} className="bg-[#140505] rounded-lg border border-[#1A0505] px-3 py-2 flex justify-between items-center shadow-inner w-full group-hover:border-[#CC0000]/30 transition-colors">
                <span className="text-[11px] font-bold text-[#A7ADBE] group-hover:text-white transition-colors truncate">
                  Cap. {cap.numero}
                </span>
                <span className={`text-[9px] font-black uppercase tracking-widest shrink-0 ml-2 ${timeLabel === 'NOVO' ? 'text-[#00FF88]' : 'text-[#777]'}`}>
                  {timeLabel}
                </span>
              </div>
            );
          ...
          <div className="bg-[#140505] rounded-lg border border-[#1A0505] px-3 py-2 flex justify-between items-center shadow-inner w-full">
            <span className="text-[11px] font-bold text-[#A7ADBE]">Sem Capítulos</span>
          </div>
        )}
      </div>
    </div>
...
const HomeView = React.memo(({ carouselData, obrasDestaque, obrasRecentes, obrasAtualizadas, currentSlide, setSaveModal, onMangaClick }) => {
  ...
  const atualizacoesFiltradas = obrasAtualizadas.filter(obra => {
    ...
    const tipoObra = normalizeString(obra.tipo);
    const tipoFiltro = normalizeString(filtro);
    return tipoObra.includes(tipoFiltro);
  });

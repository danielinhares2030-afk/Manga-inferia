  ...
  return (
    <div className="min-h-screen bg-[#030305] flex items-center justify-center p-4 relative font-nunito overflow-hidden">
      ...
      <div className={`w-full max-w-lg bg-[#0A0505]/90 backdrop-blur-xl border ${isLogin ? 'border-[#2A0A0A]' : 'border-[#7A3CFF]/30'} rounded-3xl p-10 md:p-12 shadow-[0_0_50px_rgba(0,0,0,0.5)] z-10 animate-in fade-in slide-in-from-bottom-5 transition-all duration-700`}>
        ...
        {error && <div className="bg-[#CC0000]/10 border border-[#CC0000]/30 text-[#FF3333] text-sm p-4 rounded-xl mb-6 text-center font-bold animate-in fade-in">{error}</div>}

        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="relative">
            <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-[#A7ADBE]" size={20} />
            <input
              type="email" placeholder="E-mail" required value={email} onChange={(e) => setEmail(e.target.value)}
              className={`w-full bg-[#140505] border border-[#2A0A0A] text-white text-base rounded-2xl py-4 pl-14 pr-4 focus:outline-none focus:border-${isLogin ? '[#CC0000]' : '[#7A3CFF]'} transition-colors duration-500`}
            />
          </div>
          <div className="relative">
            <Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-[#A7ADBE]" size={20} />
            <input
          ...
          <button disabled={loading} className={`w-full text-white font-bold py-4 rounded-2xl text-base uppercase tracking-wider flex items-center justify-center gap-2 hover:scale-[1.02] transition-all duration-500 disabled:opacity-50 mt-4 ${isLogin ? 'bg-gradient-to-r from-[#CC0000] to-[#990000] shadow-[0_0_20px_rgba(204,0,0,0.4)]' : 'bg-gradient-to-r from-[#7A3CFF] to-[#5A28CC] shadow-[0_0_20px_rgba(122,60,255,0.4)]'}`}>
            {loading ? <Loader2 className="animate-spin" size={20}/> : isLogin ? <><LogIn size={20} /> ENTRAR NO ABISMO</> : <><UserPlus size={20} /> CRIAR CONTA</>}
          </button>
        </form>

        <div className="mt-8 text-center">
          <button type="button" onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-[#A7ADBE] hover:text-white text-sm font-bold transition-colors">

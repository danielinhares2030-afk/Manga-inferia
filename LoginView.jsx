...
const LoginView = () => {
  ...
  return (
    <div className={`fixed inset-0 z-[9000] flex items-center justify-center p-4 transition-all duration-1000 ease-in-out bg-[#050202]`}>
      ...
      <div className={`w-full max-w-md backdrop-blur-3xl border rounded-2xl p-8 relative z-10 transition-all duration-1000 ease-in-out shadow-2xl ${loginMode === 'register' ? 'bg-[#000814]/80 border-[#00E5FF]/20 shadow-[0_20px_60px_rgba(0,229,255,0.1)]' : 'bg-[#0A0505]/80 border-[#CC0000]/20 shadow-[0_20px_60px_rgba(204,0,0,0.15)]'}`}>
        <h1 className={`font-anime text-2xl md:text-3xl text-center mb-2 drop-shadow-lg transition-colors duration-1000 ease-in-out`}>
          MANGA<span className={loginMode === 'register' ? 'text-[#00E5FF]' : 'text-[#CC0000]'}>INFERIA</span>
        </h1>
        <p className="text-center text-[#A7ADBE] text-sm mb-8 font-nunito font-bold">
        ...
        <button onClick={handleAnonAuth} disabled={isSubmitting} className="w-full bg-[#050508] border border-[#2A2A35] text-[#A7ADBE] py-3 rounded-xl font-bold text-sm hover:bg-[#1A1A24] hover:text-white transition-colors flex items-center justify-center gap-2 font-nunito">

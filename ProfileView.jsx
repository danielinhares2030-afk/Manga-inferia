...
const ProfileView = React.memo(({ perfil = {}, biblioteca = [], setActiveTab = () => {}, onMangaClick }) => {
  ...
  return (
    <div className="animate-in fade-in duration-300 font-nunito pb-10 min-h-screen relative">
      ...
      {/* INJEÇÃO DO TÍTULO NO PERFIL SE EQUIPADO */}
      {eq.titulo?.css && (
        <style dangerouslySetInnerHTML={{__html: 
          eq.titulo.uniqueClass 
            ? `.${eq.titulo.uniqueClass} { ${eq.titulo.css} } \n ${eq.titulo.animacao || eq.titulo.keyframes || ''}` 
            : `.${eq.titulo.id} { ${eq.titulo.css} } \n ${eq.titulo.animacao || eq.titulo.keyframes || ''}`
        }} />
      )}
      ...
      <div className="px-6 relative flex flex-col md:flex-row items-start md:items-end gap-5 -mt-20 md:-mt-24 z-10 mb-6">
        ...
        <div className="flex-1 mt-2 md:mt-0 pt-2 w-full">
          <div className="flex items-center justify-between w-full mb-1">
            <div>
              {/* O NOME DO USUÁRIO RECEBE A CLASSE DO TÍTULO EQUIPADO AQUI */}
              <h2 className={`font-anime text-2xl md:text-3xl font-bold tracking-wider leading-none drop-shadow-md text-[#F5F7FF] ${eq.titulo ? (eq.titulo.uniqueClass || eq.titulo.id) : ''}`}>
      ...
      {/* MODAL DE EDIÇÃO DE VOLTA AO NORMAL */}
      {editProfileModal && (

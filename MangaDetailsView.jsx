...
const MangaDetailsView = ({ obra, onBack, onReadChapter, setSaveModal }) => {
  ...
  useEffect(() => {
    // Busca os capítulos reais da sua API
    const carregarCapitulos = async () => {
      try {
        const res = await fetch(`${API_URL}/obras/${obra.id}/capitulos`);
        if (res.ok) {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-10 duration-500 bg-[#050508] min-h-screen pb-24 font-nunito">
      ...
      <div className="px-5 relative -mt-32 z-10">
        <div className="flex gap-4 items-end">
          ...
          <div className="flex-1 pb-2">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-sm font-bold text-yellow-500">{obra.rating}</span>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={() => capitulos.length > 0 && onReadChapter(capitulos[capitulos.length - 1])} disabled={capitulos.length === 0} className="flex-1 bg-gradient-to-r from-[#CC0000] to-[#7A0000] text-white py-3.5 rounded-xl font-bold font-teko text-xl flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(204,0,0,0.4)] disabled:opacity-50 tracking-wider">
            <Play size={20} fill="currentColor" /> {capitulos.length > 0 ? 'COMEÇAR A LER' : 'SEM CAPÍTULOS'}
          </button>
        ...
        <div className="mt-8">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {capitulos.map((cap) => (
                <div key={cap._id || cap.id} onClick={() => onReadChapter(cap)} className="bg-[#140505] border border-[#2A0A0A] p-4 rounded-xl flex items-center justify-between hover:border-[#CC0000]/50 cursor-pointer transition-colors group">

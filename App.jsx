...
const AppContent = () => {
  ...
  const [previousTab, setPreviousTab] = useState('home'); // LEMBRA DE ONDE O USUÁRIO VEIO
  const [selectedObraId, setSelectedObraId] = useState(null);
  const [selectedCapitulo, setSelectedCapitulo] = useState(null);

  const [toastMsg, setToastMsg] = useState(''); // MENSAGEM DE AVISO
  ...
  // FUNÇÃO GLOBAL DE AVISO (Disponível para o SaveModal)
  useEffect(() => {
    window.mostrarAviso = (msg) => {
      setToastMsg(msg);
      setTimeout(() => setToastMsg(''), 3000); // Some após 3 segundos
    };
  }, []);

  // INTERCEPTAR O BOTÃO VOLTAR DO CELULAR (Evita fechar o site)
  useEffect(() => {
    window.history.pushState(null, '', window.location.href);

    const handlePopState = () => {
      window.history.pushState(null, '', window.location.href); // Bloqueia saída
      
      if (activeTab === 'reader') {
        setActiveTab('details'); // Do leitor volta pros detalhes
      } else if (activeTab === 'details') {
        setActiveTab(previousTab); // Dos detalhes volta pra onde veio (Home ou Catálogo)
      } else if (activeTab !== 'home') {
        setActiveTab('home'); // De outras abas volta pra home
      }
    };
  ...
  // SISTEMA DE NAVEGAÇÃO ATUALIZADO (Guarda de onde veio)
  const handleMangaClick = (id) => {
    if (activeTab === 'home' || activeTab === 'catalog' || activeTab === 'biblioteca') {
      setPreviousTab(activeTab); // Salva se estava no Home ou Catálogo
    }
  ...
  return (
    <div className="min-h-screen bg-[#050508] text-[#F5F7FF] font-sans selection:bg-[#990000] selection:text-white relative overflow-x-hidden">
      ...
      {/* MENSAGEM FLUTUANTE DE AVISO (TOAST) */}
        ...
        <>
          ...
          <main className={isFullScreenView ? "pb-0" : "pb-24"}>
            <Suspense fallback={<div className="flex h-screen items-center justify-center"><Loader2 className="animate-spin text-[#CC0000] w-12 h-12" /></div>}>
              ...
              {activeTab === 'details' && <MangaDetailsView obra={obras.find(o => o.id === selectedObraId)} onBack={() => setActiveTab(previousTab)} onReadChapter={handleReadChapter} setSaveModal={setSaveModal} />}
